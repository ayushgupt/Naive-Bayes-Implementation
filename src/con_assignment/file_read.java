...
public class file_read 
{
	public static void main ( String args [])
	{
		...
		//tc gives total count and ac,bc,cc gives count of each article
		//s array store individual string which are different
		//aoc,boc of the corresponding index gives the occurance in that category 
		...
		try 
		{
			...
			while ((actionString = br.readLine()) != null) 
			{
				...
				if(tokens[0].equals("talk.politics.mideast"))
				{
					...
					{
						//see if this string is already in the array
						int drum=0;
						for(int j=0;j<adc;j++)
						{
						...
						//string is not in the array so add the string now
						if(drum==0)
						{
							s[adc]=tokens[i];
							aoc[adc]+=1;
		...
		//increasing by one if any occurance is 0
		for(int i=0;i<adc;i++)
		{
			if(aoc[i]==0)
			{
